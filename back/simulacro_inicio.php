...
document.addEventListener("DOMContentLoaded", function(){
    ...
    // Cargar módulos al seleccionar grupo
    ...
    // Cargar tipos al seleccionar módulo
    ...
    // Calcular cantidad máxima y tiempo al seleccionar tipo de prueba
    tipoSelect.addEventListener("change", function(){
        ...
        .then(data => {
            if(data.max){
                // Cantidad fija (no modificable)
                cantidadInput.value = data.max;

                // Tiempo fijo = cantidad * 1 minuto
    ...
    <div class="content">
        <div class="contenedor-derecho" style="max-width:820px">
            ...
            <form action="procesar_simulacro.php" method="post">
                <label for="grupo">Grupo de referencia:</label>
                <select id="grupo" name="grupo" required>
                    <option value="">-- Seleccione --</option>
                    <?php
                    $result = $conn->query("SELECT DISTINCT grupo FROM preguntas WHERE grupo <> '' ORDER BY grupo ASC");
                    while ($row = $result->fetch_assoc()) {
                        echo '<option value="'.htmlspecialchars($row['grupo']).'">'.htmlspecialchars($row['grupo']).'</option>';
                    }
                    ?>
                </select>
